...
![DockersDiff](https://www.jpkeisala.com/wp-content/uploads/2016/11/virtual-machine-compare-docker.jpg)
...
![KataContainers](https://katacontainers.io/media/uploads/katacontainers/images/data.jpg)
...
Log and connect to our system ```betatun.ugr.es``` with:
...
So, we execute a simple container with NGINX:


```
docker run -d -p  <PORT_ASSIGNED>:80 --ip "IP_ASSIGNED" --privileged=true --restart always --name "NAME_CONT" --net "dockernet02"  nginx
```

With the following data:

- In ``<PORT_ASSIGNED>`` write your individually assigned port.
- In ``<IP_ASSIGNED>`` write your individually assigned IP for containers.
- In ``<NAME_CONT>`` write the name of your container including your ID (i.e. ``CS7883..._nginx``).

Review your assignments here [Architecture and start-up of the training work environment.](../Practice/architecture.md)
...
In our case, due to the server configuration, we will use this template:

```
docker run -d -p  <PORT>:80 --ip "IP_CONTAINER" --privileged=true --restart always --name "Your Name" --net "dockernet02" <container>
```

Example (use your PORT, IP_CONTAINER and NAME)

```
docker run -d -p  15350:80 --ip "10.2.0.50" --privileged=true --restart always --name TestUsersCSec --net "dockernet02"  boxedcode/alpine-nginx-php-fpm
```

It will run nginx at 15350 external port ant connect it with internal container port 80. The name of container will be testnginx.
